    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            <li class="with-x" v-show="searchParmas.categoryName">
              {{ searchParmas.categoryName
              }}<i @click="removeCategoryName">×</i>
            </li>
            <li class="with-x" v-show="searchParmas.keyword">
              {{ searchParmas.keyword }}<i @click="removeKeyword">×</i>
            </li>
            <li class="with-x" v-show="searchParmas.trademark">
...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      //如果服务器要求字段可有可无，则将字段设置成undefined，当前字段不会带给服务器
      ...
      //地址栏也需要改，进行路由跳转（跳转到当前组件）
      //这里面包屑是query参数，还需要带上parmas参数
    ...
    changeOrder(flag) {
      let originOrde = this.searchParmas.order.split(':')[0];
      let originSort = this.searchParmas.order.split(':')[1];
      if (flag === originOrde ){
        this.searchParmas.order = `${originOrde}:${originSort==='desc'? 'asc':'desc'}`
      } else {
  ...
  computed: {
    ...
    isOne() {
      return this.searchParmas.order.includes("1");
    },
    isTwo() {
      return this.searchParmas.order.includes("2");
    },
    isAsc() {
      return this.searchParmas.order.includes("asc");
    },
    isDesc() {
